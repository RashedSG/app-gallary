...
export function boot3D(view, span=8, bg=0x070d1a){
  ...
  const fill=new THREE.DirectionalLight(0x7dd3fc,.28); fill.position.set(-10,8,-6); scene.add(fill);
  const ground=new THREE.Mesh(new THREE.PlaneGeometry(span*4,span*4), new THREE.ShadowMaterial({opacity:.38}));
  ...
  function fit(){const w=view.clientWidth,h=view.clientHeight||1; renderer.setSize(w,h); const a=w/h; camera.left=-span*a; camera.right=span*a; camera.top=span; camera.bottom=-span; camera.updateProjectionMatrix();}
  addEventListener('resize',fit); fit();
  return {renderer,scene,camera,fit,sun};
}

export function bootSide(view, span=8, bg=0x071824){
  const {renderer,scene,camera,fit,sun}=boot3D(view,span,bg);
  camera.up.set(0,1,0);
